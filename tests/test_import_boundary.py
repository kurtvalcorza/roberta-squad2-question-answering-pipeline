...
from roberta_question_answering_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"architectures": ["RobertaForQuestionAnswering"], "model_type": "roberta"}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        RoBERTaQuestionAnsweringPipeline.from_pretrained(
